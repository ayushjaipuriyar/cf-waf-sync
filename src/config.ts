...
const envSchema = z.object({
  CF_API_TOKEN: z.string().min(1, "Missing CF_API_TOKEN (wrangler secret put CF_API_TOKEN)"),
  CF_ACCOUNT_ID: z
    .string()
    .min(1, "Missing CF_ACCOUNT_ID (wrangler secret put CF_ACCOUNT_ID)")
    .regex(/^[a-f0-9]{32}$/i, "CF_ACCOUNT_ID must be 32-char hex (dash.cloudflare.com/<id>/)"),
...
function parseIntBound(raw: string | undefined, fallback: number, min: number, max: number, field: string): number {
  if (!raw) return fallback;
  const n = parseInt(raw, 10);
  if (Number.isNaN(n)) throw new Error(`${field} must be integer, got "${raw}"`);
  if (n < min || n > max) throw new Error(`${field} must be ${min}-${max}, got ${n}`);
  return n;
}
...
export function resolveConfig(env: Env): ResolvedConfig {
  ...
  const sniffcatConfidenceMin = parseIntBound(parsed.SNIFFCAT_CONFIDENCE_MIN, 80, 0, 100, "SNIFFCAT_CONFIDENCE_MIN");
  const sniffcatLimit = parseIntBound(parsed.SNIFFCAT_LIMIT, 2000, 1, 10000, "SNIFFCAT_LIMIT");
  const abuseIpdbConfidenceMin = parseIntBound(parsed.ABUSEIPDB_CONFIDENCE_MIN, 75, 0, 100, "ABUSEIPDB_CONFIDENCE_MIN");
  const abuseIpdbLimit = parseIntBound(parsed.ABUSEIPDB_LIMIT, 8000, 1, 10000, "ABUSEIPDB_LIMIT");

  const zoneIds = splitCsv(parsed.CF_ZONE_IDS);
  const zoneNames = splitCsv(parsed.CF_ZONE_NAMES);
  if (zoneIds.length && zoneNames.length && zoneIds.length !== zoneNames.length && zoneNames.length !== 1) {
    throw new Error(`CF_ZONE_IDS (${zoneIds.length}) and CF_ZONE_NAMES (${zoneNames.length}) length mismatch — provide 1 name or N names matching N ids`);
  }
  // validate zone ids are 32 hex or known alias like seabuddy id
  for (const id of zoneIds) {
    if (!/^[a-f0-9]{32}$/i.test(id)) throw new Error(`CF_ZONE_IDS entry "${id}" must be 32-char hex`);
  }
  ...
  return {
    ...
    listName: parsed.CF_IP_BLOCKLIST_NAME === "" ? "" : parsed.CF_IP_BLOCKLIST_NAME || "sefinek_cf_waf",
    phpSupport: php,
    wordpressSupport: wp,
...
export function validateConfig(cfg: ResolvedConfig): string[] {
  ...
  if (cfg.sniffcatLimit + cfg.abuseIpdbLimit > 10000) {
    warnings.push(`SNIFFCAT_LIMIT (${cfg.sniffcatLimit}) + ABUSEIPDB_LIMIT (${cfg.abuseIpdbLimit}) > 10000 Free cap — budget will trim AbuseIPDB tail`);
  }
  if (cfg.allowlist && cfg.allowlist.length > 2048) {
    warnings.push(`ALLOWLIST ${cfg.allowlist.length} chars may push expressions over MAX 4096`);
  }
  return warnings;
}
