...
import { getOrCreateList, resolveMaxListSize, syncListItems } from "./cloudflare/lists";
import { fetchSniffCatIPs } from "./integrations/sniffcat";
import { fetchAbuseIPDBIPs } from "./integrations/abuseipdb";
import pLimit from "p-limit";

export interface SyncResult {
  zones: Array<{ name: string; status: string; details: string; diff?: { before: number; after: number; changes: string[] } }>;
...
// ---------- structured logger ----------
function jlog(level: "info" | "warn" | "error", msg: string, extra: Record<string, unknown> = {}) {
...
function secureCompare(a: string, b: string): boolean {
  ...
  try {
    ...
    // @ts-ignore workers have timingSafeEqual on crypto
    if ((crypto as any).timingSafeEqual) return (crypto as any).timingSafeEqual(ab, bb);
  } catch {}
  let out = 0;
...
async function syncIPList(cfg: ReturnType<typeof resolveConfig>): Promise<{ added: number; removed: number; total: number } | null> {
  ...
  if (truncatedCount > 0) {
    jlog("warn", `List cap exceeded: ${priorityIPs.length + newAbuse.length} unique but only ${maxListSize} — dropped ${truncatedCount}`, {
      truncatedCount,
...
function buildDiff(current: any[], desired: any[]): string[] {
  ...
  for (const r of desired) if (curMap.has(r.description) && curMap.get(r.description) !== r.expression) changes.push(`~ ${r.description} (expression changed)`);
  return changes;
}
...
async function syncOneZone(
  ...
  dryRun: boolean,
  existingManagedCache?: any[]
): Promise<{ name: string; status: string; details: string; diff?: { before: number; after: number; changes: string[] } }> {
  const allowlistExpr = buildZoneExpression(allowlistEntries as any, zone as any);
  const wrap = (expr: string) => (allowlistExpr ? `not (${allowlistExpr}) and (${expr})` : expr);
  const blocklistExpr = buildZoneExpression(blocklistEntries as any, zone as any);
  ...
  try {
    ...
    const diff = { before: current.length, after: desired.length, changes: diffChanges };

    if (normalizedEqual) {
      jlog("info", `${zone.name}: Up to date`, { before: current.length, after: desired.length });
      return { name: zone.name, status: "Up to date", details: "-", diff };
    }

    if (dryRun) {
      jlog("info", `[dryRun] ${zone.name}: would update`, { managed: partRules.length, preserved: userRules.length });
      return { name: zone.name, status: "Would update", details: `${partRules.length} managed, ${userRules.length} preserved, blocklist: ${blocklistStatus}`, diff };
    }

    await putEntrypoint(cfg, zone.id, desired);
    jlog("info", `${zone.name}: Updated`, { managed: partRules.length, preserved: userRules.length });
    ...
    if (cfg.discordWebhookUrl) {
      try {
        await fetch(cfg.discordWebhookUrl, {
          ...
          body: JSON.stringify({ content: `✅ WAF updated: ${zone.name} (${partRules.length} managed, ${userRules.length} preserved)` }),
        });
      } catch {}
    }
    return { name: zone.name, status: "Updated", details: `${partRules.length} managed, ${userRules.length} preserved`, diff };
  } catch (err: any) {
    ...
    if (msg.includes("is unknown") && msg.includes("list") && cfg.listName) {
      jlog("warn", `${zone.name}: list ${cfg.listName} unknown with this token (needs cfat_...), retrying without IP list`, { msg });
      try {
        ...
        const allowlistExpr2 = buildZoneExpression(allowlistEntries as any, zone as any);
        const wrap2 = (expr: string) => (allowlistExpr2 ? `not (${allowlistExpr2}) and (${expr})` : expr);
        ...
        if (normalize(current) === normalize(retryDesired)) {
          return { name: zone.name, status: "Up to date (without IP list)", details: "list unavailable with cfut", diff: { before: current.length, after: retryDesired.length, changes: diffChanges } };
        }
        if (dryRun) {
          return { name: zone.name, status: "Would update (without IP list)", details: `${retryPartRules.length} managed`, diff: { before: current.length, after: retryDesired.length, changes: diffChanges } };
        }
        await putEntrypoint(cfg, zone.id, retryDesired);
        return { name: zone.name, status: "Updated (without IP list)", details: `${retryPartRules.length} managed — provide cfat_... to enable ${cfg.listName}`, diff: { before: current.length, after: retryDesired.length, changes: diffChanges } };
      } catch (retryErr: any) {
        ...
        return { name: zone.name, status: "Error", details: `${msg} | retry without list also failed: ${rmsg}` };
      }
    }
...
export async function runSync(env: Env, dryRun = false, triggerCron?: string): Promise<SyncResult> {
  ...
  if (!expressions || !Object.keys(expressions).filter((k) => k !== "_meta").length) throw new Error("No expressions found — check RULES_SOURCE_URL/bundled");
  ...
  if (!dryRun) {
    try {
      ...
      if (msg?.includes("Authentication error") || err.status === 403) {
        jlog("warn", `IP list sync skipped (token lacks Account Filter Lists:Edit — use cfat_... or set CF_IP_BLOCKLIST_NAME="" to silence)`, { msg });
        ipResult = null;
      } else {
        throw err;
      }
  ...
  jlog("info", `Found ${zones.length} zones, ${filtered.length} active after excluded filter`);

  // Concurrent zones with limit 5
  const limit = pLimit(5);
  const zoneTasks = filtered.map((zone) => limit(() => syncOneZone(zone, cfg, expressions, blocklistHostIndex, allowlistEntries, blocklistEntries, dryRun)));
  const results = await Promise.all(zoneTasks);

  const durationMs = Date.now() - start;
  jlog("info", `Done in ${durationMs}ms`, { cron: triggerCron ?? "manual", zones: results.length, ipList: ipResult?.total });

  // KV audit — best effort
  if ((env as any).SYNC_KV) {
    try {
      const kv = (env as any).SYNC_KV as KVNamespace;
      await kv.put("lastSync", JSON.stringify({ at: new Date().toISOString(), durationMs, cron: triggerCron, result: { zones: results, ipList: ipResult, warnings } }), { expirationTtl: 60 * 60 * 24 * 7 });
      await kv.put("lastSync:raw", JSON.stringify({ meta: { version: metaVersion, durationMs, cron: triggerCron }, zones: results, ipList: ipResult }), { expirationTtl: 60 * 60 * 24 * 7 });
    } catch (err: any) {
      jlog("warn", "KV audit write failed", { error: err.message });
    }
  }

  return { zones: results, ipList: ipResult ? { added: ipResult.added, removed: ipResult.removed, total: ipResult.total } : undefined, warnings, meta: { version: metaVersion, durationMs, cron: triggerCron } };
}

export default {
  async scheduled(controller: ScheduledController, env: Env, ctx: ExecutionContext) {
    try {
      jlog("info", `scheduled cron: ${controller.cron} at ${new Date(controller.scheduledTime).toISOString()}`);
      await runSync(env, false, controller.cron);
    } catch (err: any) {
      jlog("error", `scheduled failed`, { error: err.message, stack: err.stack });
      throw err;
    }
  },
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      ...
      return new Response(JSON.stringify({ ok: true, service: "cf-waf-sync", version: "1.0.0", lastSync, now: new Date().toISOString() }, null, 2), { headers: { "Content-Type": "application/json" } });
    }

    if (url.pathname === "/sync" || url.pathname === "/") {
      const dryRun = url.searchParams.get("dryRun") === "1" || url.searchParams.get("dry_run") === "1" || url.searchParams.get("diff") === "1" || env.DRY_RUN === "true";
      const wantDiff = url.searchParams.get("diff") === "1" || url.searchParams.get("dryRun") === "diff";

      // Rate limit: 10 req / min per IP for /sync
      const rlKey = `rl:${ip}:${url.pathname}`;
      if (!checkRateLimit(rlKey, 10, 60_000)) {
        return new Response(JSON.stringify({ error: "Too many requests — rate limited (10/min)" }), { status: 429, headers: { "Content-Type": "application/json", "Retry-After": "60" } });
      }
      ...
      const isAuthed = auth.length === expected.length && secureCompare(auth, expected);
      ...
      if (!isAuthed && !allowUnauthDryRun) {
        if (request.method !== "GET" || !dryRun) {
          return new Response(JSON.stringify({ error: "Unauthorized — send Authorization: Bearer $CF_API_TOKEN" }), { status: 401, headers: { "Content-Type": "application/json", "WWW-Authenticate": 'Bearer realm="cf-waf-sync"' } });
        }
      }
      ...
      try {
        ...
        if (wantDiff) {
          return new Response(JSON.stringify({ success: true, dryRun, diff: result.zones.map((z) => ({ name: z.name, status: z.status, diff: z.diff, details: z.details })), result }, null, 2), {
            headers: { "Content-Type": "application/json" },
          });
        }
    ...
    return new Response("Not found. Use GET /health or GET /sync?dryRun=1 or GET /sync?diff=1", { status: 404 });
  },
