...
export const isManagedDescription = (desc: string) =>
  PART_REGEX.test(desc) || desc === BLOCKLIST_DESCRIPTION
export const isPartDescription = (desc: string, index: number) =>
  new RegExp(`Part ${index}\\b`).test(desc || "")

export const RULE_CAPS: Record<string, number> = {
  free: 5,
  lite: 5,
  pro: 20,
  business: 100,
...
export function buildZoneExpression(
  entries: ZoneScopedEntry[],
  zone: Zone,
): string | null {
