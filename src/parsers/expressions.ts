...
export type ExpressionsMap = Record<number, ExpressionBlock> & {
  _meta: { version: string | null; totalLength: number; blocks: number }
}

const PATTERN =
  /##\s*([^<\n]*)[^\n]*\n> \*\*Action:\*\* ([^\n]*)\n+```([\s\S]*?)```/gi

export function parseExpressionsText(
  raw: string,
  cfg: ResolvedConfig,
): ExpressionsMap | null {
  ...
  const blocks = [...text.matchAll(PATTERN)].map(
    ([, name, action, rawExpressions]) => {
  ...
  blocks.forEach((block, i) => {
    if (
      !block.expressions ||
      block.expressions.trim() === "" ||
      block.expressions.trim() === "(true)"
    ) {
      droppedEmpty++
      console.warn(
        `Dropped empty expression block ${i + 1} "${block.name}" after PHP/WP filtering — would have been block-all`,
      )
      return
    }
    if (block.expressions.length > 4096) {
      console.warn(
        `Block ${i + 1} "${block.name}" ${block.expressions.length} chars exceeds MAX 4096 — will error per zone`,
      )
    }
    acc[i + 1] = block
  })
  if (droppedEmpty)
    console.warn(
      `Dropped ${droppedEmpty} empty blocks — check PHP_SUPPORT/WORDPRESS_SUPPORT filters`,
    )
...
export async function loadExpressions(
  cfg: ResolvedConfig,
  bundledText: string,
): Promise<ExpressionsMap | null> {
  let text = bundledText
  if (cfg.rulesSourceUrl) {
    const res = await fetch(cfg.rulesSourceUrl)
    if (!res.ok)
      throw new Error(
        `Failed to fetch RULES_SOURCE_URL ${cfg.rulesSourceUrl}: ${res.status}`,
      )
    text = await res.text()
  }
