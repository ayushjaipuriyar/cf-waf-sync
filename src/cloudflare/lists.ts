...
export async function getAllLists(cfg: ResolvedConfig): Promise<CFList[]> {
  const data = await cfFetch<CFList[]>(
    cfg,
    `/accounts/${cfg.cfAccountId}/rules/lists`,
  )
  return data.result
}

export async function isEnterpriseAccount(
  cfg: ResolvedConfig,
): Promise<boolean> {
  try {
    const data = await cfFetch<any[]>(
      cfg,
      `/accounts/${cfg.cfAccountId}/subscriptions`,
    )
    return (data.result || []).some(
      (sub: any) =>
        sub.rate_plan?.id === "enterprise" ||
        sub.rate_plan?.is_contract === true,
    )
  } catch {
    return false
  }
...
export function resolveMaxListSize(
  allLists: CFList[],
  ...
  const usedByOthers = allLists
    .filter(
      (l) => l.id !== ownListId && l.kind !== "hostname" && l.kind !== "asn",
    )
...
export async function getOrCreateList(
  cfg: ResolvedConfig,
): Promise<{ listId: string; allLists: CFList[]; itemCap: number }> {
  if (!cfg.listName) throw new Error("listName empty")
  const [allLists, enterprise] = await Promise.all([
    getAllLists(cfg),
    isEnterpriseAccount(cfg),
  ])
  ...
  const listSources = [
    "ip-blocklist.txt",
    ...(cfg.sniffcatToken ? ["SniffCat"] : []),
    ...(cfg.abuseIpdbKey ? ["AbuseIPDB"] : []),
  ]
  ...
  const list = allLists.find((l) => l.name === cfg.listName)
  if (list) {
    if (list.description !== description) {
      await cfFetch(
        cfg,
        `/accounts/${cfg.cfAccountId}/rules/lists/${list.id}`,
        {
          method: "PUT",
...
async function waitForBulk(cfg: ResolvedConfig, operationId: string) {
  for (let attempt = 0; attempt < 30; attempt++) {
    const data = await cfFetch<{ status: string; error?: string }>(
      cfg,
      `/accounts/${cfg.cfAccountId}/rules/lists/bulk_operations/${operationId}`,
    )
    const status = (data.result as any).status
    const error = (data.result as any).error
    if (status === "completed") return
    if (status === "failed")
      throw new Error(`Bulk operation failed: ${error || "unknown"}`)
...
export async function getAllListItems(
  ...
  do {
    ...
    const data = await cfFetch<CFListItem[]>(
      cfg,
      `/accounts/${cfg.cfAccountId}/rules/lists/${listId}/items`,
      { params },
    )
...
export async function syncListItems(
  ...
  const toDelete = currentItems
    .filter((it) => !desiredIPs.has(it.ip))
    .map((it) => ({ id: it.id }))

  if (toDelete.length) {
    const data = await cfFetch<{ operation_id: string }>(
      cfg,
      `/accounts/${cfg.cfAccountId}/rules/lists/${listId}/items`,
      {
        method: "DELETE",
