  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (isRetryable(res.status) && attempt < retries) {
        const retryMs = parseRetryAfter(res) ?? jitter(1000 * Math.pow(2, attempt));
        // eslint-disable-next-line no-console
        console.warn(`cfFetch ${path} ${res.status} retry ${attempt + 1}/${retries} after ${Math.round(retryMs)}ms`);
        await sleep(retryMs);
        continue;
      }
      ...
      if (!res.ok || !json.success) {
        ...
        if (isRetryable(res.status) && attempt < retries) {
          lastErr = err;
          const retryMs = jitter(1000 * Math.pow(2, attempt));
          await sleep(retryMs);
          continue;
        }
        throw err;
      ...
      if ((isAbort || isNetwork || (err.status && isRetryable(err.status))) && attempt < retries) {
        const retryMs = jitter(1000 * Math.pow(2, attempt));
        console.warn(`cfFetch ${path} ${err.message} retry ${attempt + 1}/${retries} after ${Math.round(retryMs)}ms`);
        await sleep(retryMs);
        continue;
      }
      throw err;
