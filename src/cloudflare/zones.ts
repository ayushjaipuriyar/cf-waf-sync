...
export async function getZones(cfg: ResolvedConfig): Promise<Zone[]> {
  ...
  if (cfg.zoneIds.length || cfg.zoneNames.length) {
    ...
    try {
        ...
        if (cfg.zoneIds.length) {
          for (let i = 0; i < cfg.zoneIds.length; i++) {
            ...
            if (real) matched.push(real);
              ...
              matched.push({ id, name: fallbackName, status: "active", plan: { legacy_id: legacy, name: legacy } } as Zone);
            }
          }
        } else {
          for (const name of cfg.zoneNames) {
            ...
            if (real) matched.push(real);
              ...
              matched.push({ id, name, status: "active", plan: { legacy_id: legacy, name: legacy } } as Zone);
            }
          }
        }
      ...
      if (err.status !== 403 && err.status !== 401) console.warn(JSON.stringify({ level: "warn", msg: "zone probe failed, using stub", error: err.message }));
