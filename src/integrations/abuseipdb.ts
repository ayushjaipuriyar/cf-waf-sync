...
async function fetchWithRetry(url: string, init: RequestInit, retries = 2, timeoutMs = 10000): Promise<Response> {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if ((res.status === 429 || res.status >= 500) && attempt < retries) {
        const retryMs = 500 * Math.pow(2, attempt) * (0.5 + Math.random());
...
export async function fetchAbuseIPDBIPs(cfg: ResolvedConfig): Promise<string[]> {
  ...
  try {
    ...
    const res = await fetchWithRetry(url.toString(), {
      headers: { Key: cfg.abuseIpdbKey, Accept: "application/json", "User-Agent": "cf-waf-sync/1.0" },
    });
    if (!res.ok) throw new Error(`AbuseIPDB ${res.status} ${await res.text()}`);
    const data: any = await res.json();
    return (data.data || []).map((e: any) => e.ipAddress).filter(Boolean);
  } catch (err: any) {
    console.warn(JSON.stringify({ level: "warn", msg: "AbuseIPDB fetch failed", error: err.message }));
    return [];
  }
}
